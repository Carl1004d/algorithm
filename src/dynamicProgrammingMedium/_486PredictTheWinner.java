...
public class _486PredictTheWinner {
	public boolean PredictTheWinner(int[] nums) {
		...
		int max = partition(begin, end, nums);
		if (max >= 0)
			return true;
		return false;
	}
	...
	private int partition(int begin, int end, int[] nums) {
		if (begin == end)
			return nums[begin];
		int max = Math.max(nums[begin] - partition(begin + 1, end, nums), nums[end] - partition(begin, end - 1, nums));
		return max;
	}
	...
	public static void main(String[] args) {
		_486PredictTheWinner o = new _486PredictTheWinner();
		System.out.println(o.dp2(new int[]{1,2,3,4,5}));
	}
